...
module "dns"  {
    depends_on	= [ module.alb-hana, module.alb-ascs, module.alb-ers ]
    source		= "./modules/dns"
    ZONE			= var.ZONE
    REGION  = var.REGION
    VPC			= var.VPC
...
module "s4pasreq" {
  ...
  depends_on	= [ module.app-vsi, local_file.ha_ansible_infra-vars, local_file.app_ansible_saps4app-vars, local_file.db_ansible_saphana-vars, local_file.tf_ansible_hana_storage_generated_file, module.file-shares, module.dns ]
...
module "s4aasreq" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.s4pasreq ]
  IP1			= data.ibm_is_instance.app-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/s4aasreq.yml"
}

module "hdbprimaryreq" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.s4aasreq ]
  IP1			= data.ibm_is_instance.db-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbprimaryreq.yml"
}

module "hdbreplicareq" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbprimaryreq ]
  IP1			= data.ibm_is_instance.db-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbreplicareq.yml"
}

module "sapclustersetup" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbreplicareq ]
  IP1			= data.ibm_is_instance.app-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/sapclustersetup.yml"
}

module "hdbclustersetup" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.sapclustersetup ]
  IP1			= data.ibm_is_instance.db-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbclustersetup.yml"
}

module "hdbprimaryinst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbclustersetup ]
  IP1			= data.ibm_is_instance.db-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbprimaryinst.yml"
}

module "hdbreplicainst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbprimaryinst ]
  IP1			= data.ibm_is_instance.db-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbreplicainst.yml"
}

module "ascsclusterinst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbreplicainst ]
  IP1			= data.ibm_is_instance.app-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/ascsclusterinst.yml"
}

module "ersclusterinst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.ascsclusterinst ]
  IP1			= data.ibm_is_instance.app-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/ersclusterinst.yml"
}

module "dbinstance" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.ersclusterinst ]
  IP1			= data.ibm_is_instance.app-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/dbinstance.yml"
}

module "hdbprimaryhsr" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.dbinstance ]
  IP1			= data.ibm_is_instance.db-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbprimaryhsr.yml"
}

module "hdbreplicahsr" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbprimaryhsr ]
  IP1			= data.ibm_is_instance.db-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbreplicahsr.yml"
}

module "hdbclusterprimary" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbreplicahsr ]
  IP1			= data.ibm_is_instance.db-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/hdbclusterprimary.yml"
}

module "s4pasinst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.hdbclusterprimary ]
  IP1			= data.ibm_is_instance.app-vsi-1.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/s4pasinst.yml"
}

module "s4aasinst" {
  source		= "./modules/ansible-exec"
  depends_on	= [ module.s4pasinst ]
  IP1			= data.ibm_is_instance.app-vsi-2.primary_network_interface[0].primary_ip[0].address
  PLAYBOOK_PATH = "ansible/s4aasinst.yml"
}

module "sec-exec" {
  source		= "./modules/sec-exec"
  depends_on	= [ module.s4aasinst ]
