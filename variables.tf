...
variable "ID_RSA_FILE_PATH" {
    ...
    nullable = false
    description = "File path for PRIVATE_SSH_KEY. It will be automatically generated. If it is changed, it must contain the relative path from git repo folders. Example: ansible/id_rsa_s4hana_ha"
}
...
variable "HANA_MAIN_PASSWORD" {
  type        = string
  sensitive   = true
  description = "The SAP HANA MAIN Password. Must be 8 to 14 characters long, contain at least one digit (0-9), one uppercase letter, and must not contain \\ (backslash) or \" (double quote)."
  validation {
    condition     = length(var.HANA_MAIN_PASSWORD) >= 8 && length(var.HANA_MAIN_PASSWORD) <= 14 && length(regexall("[0-9]", var.HANA_MAIN_PASSWORD)) > 0 && length(regexall("[A-Z]", var.HANA_MAIN_PASSWORD)) > 0 && length(regexall("[a-z]", var.HANA_MAIN_PASSWORD)) > 0 && length(regexall("[\\\\\"]", var.HANA_MAIN_PASSWORD)) == 0 && length(regexall("[\"]", var.HANA_MAIN_PASSWORD)) == 0
    error_message = "The HANA_MAIN_PASSWORD is not valid. It must be 8 to 14 characters long and must contain at least one digit, one uppercase letter, one lowercase letter. It must not contain \\ (backslash) or \" (double quote)."
  }
}
...
variable "SAP_MAIN_PASSWORD" {
  type        = string
  sensitive   = true
  description = "The SAP MAIN Password. Must be 8 to 14 characters long, must contain at least one digit (0-9) and one uppercase letter, and must not contain \\ (backslash) and \" (double quote)."
  validation {
    condition     = length(var.SAP_MAIN_PASSWORD) >= 8 && length(var.SAP_MAIN_PASSWORD) <= 14 && length(regexall("[0-9]", var.SAP_MAIN_PASSWORD)) > 0 && length(regexall("[A-Z]", var.SAP_MAIN_PASSWORD)) > 0 && length(regexall("[\\\\\"]", var.SAP_MAIN_PASSWORD)) == 0
    error_message = "The SAP_MAIN_PASSWORD is not valid. It must be 8 to 14 characters long and contain at least one digit and one uppercase letter. It must not contain \\ (backslash) or \" (double quote)."
  }
}
...
variable "HA_PASSWORD" {
  ...
  validation {
    condition     = length(var.HA_PASSWORD) >= 8 && length(var.HA_PASSWORD) <= 14 && length(regexall("[0-9]", var.HA_PASSWORD)) > 0 && length(regexall("[A-Z]", var.HA_PASSWORD)) > 0 && length(regexall("[\\\\\"]", var.HA_PASSWORD)) == 0
    error_message = "The HA_PASSWORD is not valid. It must be 8 to 14 characters long and contain at least one digit and one uppercase letter. It must not contain \\ (backslash) or \" (double quote)."
  }
}
